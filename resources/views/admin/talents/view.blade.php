  ...
  <div class="min-h-screen bg-gray-100 p-6">
    <div class="bg-white rounded-lg shadow-md p-4">
      <h2 class="text-2xl font-semibold mb-4">Talent Profile
        Details</h2>
      <div>
        <p><strong>Name:</strong> {{ $profile->user->name }}
        </p>
        <p><strong>Email:</strong> {{ $profile->user->email
          }}</p>
        <p><strong>Skills:</strong> {{ $profile->skills }}
        </p>
      ...
      <div class="mt-4">
        <a href="{{ route('admin.talent_management.index') }}"
          class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">Back</a>
        <a href="{{ route('admin.talent_management.edit', $profile->id) }}"
          class="bg-yellow-500 text-white px-4 py-2 rounded hover:bg-yellow-600">Edit</a>
        <form
          action="{{ route('admin.talent_management.delete', $profile->id) }}"
